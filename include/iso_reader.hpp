...
public:
    ...
    // Common
    bool open(const char *filename, unsigned int threads = 1);            // Open the source/destination file
    ...
    // Reader
    unsigned long long read(unsigned char *output, unsigned long long toRead); // Read method. Must return the original disk data, after decompress, decode...
...
protected:
    ...
    // FileIO
    std::basic_ifstream<unsigned char> input_file;
};
...
extern "C"
{
    ...
    bool SHARED_EXPORT open(void *handler, const char *filename, unsigned int threads = 1);                 // Open the source/destination file
    ...
    unsigned long long SHARED_EXPORT read(void *handler, unsigned char *output, unsigned long long toRead); // Read method. Must return the original disk data, after decompress, decode...
